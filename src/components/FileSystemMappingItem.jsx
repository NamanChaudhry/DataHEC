...
const FileSystemMappingItem = ({
  ...
  // useEffect(() => {
  //   if (selectedCategory.toLowerCase() === "header" && selectedSourceSystem) {
  //     // Build expected filename based on selected source system
  //     const expectedFileName = `${selectedSourceSystem.replace(/\./g, '')}_header`;
 
  //     // Fetch from backend uploads folder
  //     fetch("http://localhost:5001/api/uploads")
  //       .then((res) => res.json())
  //       .then((files) => {
  //         // Find matching file (case-insensitive)
  //         const matchedFile = files.find(
  //           (f) => f.toLowerCase().includes(expectedFileName.toLowerCase())
  //         );
  ...
  //     setSelectedFile("");
  //   }
  // }, [selectedCategory, selectedSourceSystem]);
  ...
  useEffect(() => {
    ...
    const sourceKey = selectedSourceSystem
      ? selectedSourceSystem.replace(/\s|\.|_/g, "").toLowerCase()
      : null;
    ...
    fetch("http://localhost:5001/api/uploads")
      .then(res => res.json())
      .then(files => {
        // Match files that contain the category and optionally the source system
        const matchedFile = files.find(file => {
          const cleanName = file.replace(/\s|\.|_/g, "").toLowerCase();
          if (sourceKey) {
            return cleanName.includes(categoryKey) && cleanName.includes(sourceKey);
          }
          return cleanName.includes(categoryKey);
        });
      ...
      .catch(err => console.error("Error fetching uploaded files:", err));
  }, [selectedSourceSystem, selectedCategory]);
          ...
          <Box flex={1}>
            ...
            {selectedFile && (
              ...
              >
                ...
                >
                  <InsertDriveFileIcon sx={{ fontSize: 24, color: "#527cb4ff", mr: 1.5 }} />
                  <Box sx={{ flexGrow: 1 }}>
                    <Typography
                      variant="body2"
                      sx={{
            ...
            >
              ...
              >
                ...
                {Array.isArray(rules) && rules.length === 0 ? (
                  ...
                  Array.isArray(rules) && rules.map((ruleObj, index) => (
